...
import debug, { enable as enableDebug, disable as disableDebug } from "debug";
import {
  empty as isEmpty,
  fn as isFunction,
  object as isObject,
...
export default class Analytics extends Emitter {
  ...
  initialize(settings, options) {
    ...
    // make ready callback
    const integrationCount = keys(integrations).length;
    let calledReady = 0;
    const ready = () => {
      calledReady++;
  ...
  trackLink(links, event, properties) {
    ...
    links.forEach(el => {
      ...
      el.addEventListener("click", e => {
        const ev = isFunction(event) ? event(el) : event;
        const props = isFunction(properties) ? properties(el) : properties;
        const href =
